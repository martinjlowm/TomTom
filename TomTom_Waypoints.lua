...
do
    ...
    function Minimap_OnUpdate(...)
        local elapsed = arg1
        local dist,x,y = Astrolabe:GetDistanceToIcon(this)
        if not dist then
            this:Hide()
            return
        end
        ...
        local edge = Astrolabe:IsIconOnEdge(this)
        ...
        if edge then
            ...
            local angle = Astrolabe:GetDirectionToIcon(this)
            angle = angle + rad_135

            local sin,cos = math.sin(angle) * square_half, math.cos(angle) * square_half
            this.arrowout:SetTexCoord(0.5-sin, 0.5+cos, 0.5+cos, 0.5+sin, 0.5-cos, 0.5-sin, 0.5+sin, 0.5-cos)
    ...
    function Minimap_OnEvent(...)
        if event == 'PLAYER_ENTERING_WORLD' then
            local data = this.data
            Astrolabe:PlaceIconOnMinimap(this, data.c, data.z, data.x/100,
                                         data.y/100)
        end
    end
end
