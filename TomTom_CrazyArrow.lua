...
do
    ...
    function DongleUtils.ColorGradient(perc, ...)
        ...
        local hexes = type(select(1, unpack(arg))) == 'string'

        if perc == 1 then
            if hexes then return select(num, unpack(arg))
            else return select(num-2, unpack(arg)), select(num-1, unpack(arg)), select(num, unpack(arg)) end
        end
        ...
        if hexes then
            r1, g1, b1 = DongleUtils.HexToRGBPerc(select(segment+1, unpack(arg)))
            r2, g2, b2 = DongleUtils.HexToRGBPerc(select(segment+2, unpack(arg)))
        else
            r1, g1, b1 = select((segment*3)+1, unpack(arg)), select((segment*3)+2, unpack(arg)), select((segment*3)+3, unpack(arg))
            r2, g2, b2 = select((segment*3)+4, unpack(arg)), select((segment*3)+5, unpack(arg)), select((segment*3)+6, unpack(arg))
        end
    ...
    do
        ...
        for k, v in pairs(t) do
            if v:IsObjectType('Model') and not v:GetName() then
                ...
                if string.match(model, '(interface\\minimap\\minimaparrow)') then
                    obj = v; break;
                end
            end
        end
...
local function OnUpdate()
    ...
    if dist <= arrive_distance then
        ...
        cell = math.fmod(floor(angle / twopi * 108 + 0.5), 108)
        local column = math.fmod(cell, 9)
        local row = floor(cell / 9)
    ...
    if time >= 1 then
        if cell <= 27 or cell >= 81 or showDownArrow then
            ...
            if delta > 0 then
                ...
                tta:SetText(sformat('%01d:%02d', eta / 60, math.fmod(eta, 60)))
            else
                tta:SetText('***')
            end
            time = 0
